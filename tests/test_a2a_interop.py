...
def test_a2a_external_client_full_cycle(cli_env):
    ...
    conn = _client(PORT)
    try:
        # --- discovery: public agent card served WITHOUT a token
        #     (discover-before-talk, A2A §6.9/§8.2) ---
        ...
        skill = next(s for s in card["skills"] if s["id"] == "audit")
        assert skill["name"] == "audit"
        # the auth mechanism is discoverable: X-Synapse-Token as an
        # A2A v1.0 ApiKey SecurityScheme — a name->scheme OBJECT (the
        # official SDK iterates it with .values()), plus securityRequirements

...
def test_my_work_text_output_shows_tasks(cli_env):
    """`task my-work` text mode must display the work queue: it read the
    key \"tasks\" while the API returns \"work_items\" — the default
    output was always empty."""
    _, _, env = cli_env
    _bootstrap(env)
    try:
        proc = run_cli(env, "task", "create", "Visible work", "--assignee", "data",
                       "--json")
        assert proc.returncode == 0, proc.stderr.decode()
        proc = run_cli(env, "task", "my-work", "--my-name", "data",
                       "--password-stdin", stdin="motdepasse-data-1\n")
        assert proc.returncode == 0, proc.stderr.decode()
        out = proc.stdout.decode()
        assert "Visible work" in out, f"work item missing from text output: {out}"
        assert "work_items" not in out  # raw JSON is not dumped
    finally:
        run_cli(env, "server", "stop")


def test_notifications_text_output_shows_unread_by_sender(cli_env):
    """`message notifications` text mode must show the unread-by-sender
    section: it read the key \"unread\" while the API returns the
    \"unread_by_sender\" dict — the section was never displayed."""
    _, _, env = cli_env
    _bootstrap(env)
    try:
        run_cli(env, "message", "send", "data", "hello from acme", "--my-name",
                "acme_humain", "--password-stdin", stdin="motdepasse-acme-1\n")
        proc = run_cli(env, "message", "notifications", "--my-name", "data",
                       "--password-stdin", stdin="motdepasse-data-1\n")
        assert proc.returncode == 0, proc.stderr.decode()
        out = proc.stdout.decode()
        assert "acme_humain" in out, f"sender missing from notifications: {out}"
        assert "unread_by_sender" in out
    finally:
        run_cli(env, "server", "stop")


def test_org_structure_text_output_shows_member_roles(cli_env):
    """`org structure` text mode must show the per-member role and the
    unassigned agents: it read a non-existent 'role' key on the
    department (always '—') and dumped member dicts raw."""
    _, _, env = cli_env
    _bootstrap(env, extra_agents=("data",))
    try:
        proc = run_cli(env, "api", "create_department", "--department-name", "support")
        assert proc.returncode == 0, proc.stderr.decode()
        proc = run_cli(env, "agent", "department", "data", "support",
                       "--role", "manager", "--json")
        assert proc.returncode == 0, proc.stderr.decode()
        proc = run_cli(env, "org", "structure", "acme")
        assert proc.returncode == 0, proc.stderr.decode()
        out = proc.stdout.decode()
        assert "support" in out
        assert "data (manager)" in out, f"member role missing: {out}"
        assert "{'username'" not in out  # raw dicts are not dumped
    finally:
        run_cli(env, "server", "stop")
...
def test_a2a_daemon_token_not_in_argv(cli_env, monkeypatch):
    """The bridge access token must travel through the stdin pipe, never
    as a command-line argument (visible in ps/proc)."""
    import synapse.cli.a2a as a2a_mod
    ...
    class _FakeProc:
        def __init__(self, cmd, stdin=None, **kwargs):
            captured["cmd"] = cmd
            # subprocess.PIPE is replaced by a real pipe in Popen;
            # simulate that here.
            self.stdin = _FakeStdin() if stdin is a2a_mod.subprocess.PIPE else stdin
    ...
    monkeypatch.setattr(a2a_mod.subprocess, "Popen", _FakeProc)
    # avoid the "already running" short-circuit
    monkeypatch.setattr(a2a_mod, "read_pid_file", lambda *a, **k: None)
    monkeypatch.setattr(a2a_mod, "wait_ready", lambda *a, **k: True)
    monkeypatch.setattr(a2a_mod, "_a2a_responding", lambda *a, **k: True)
    # resolve_org_auth is not needed: socket_responds must be bypassed
    monkeypatch.setattr(a2a_mod, "socket_responds", lambda *a, **k: True)
    monkeypatch.setattr(a2a_mod, "resolve_config", lambda *a, **k: None)
    monkeypatch.setattr(a2a_mod, "getpass_get", lambda prompt: "motdepasse-agent-1")

    class _FakeSysStdin:
        def readline(self):
            return "jeton-secret-123\n"

    monkeypatch.setattr(a2a_mod.sys, "stdin", _FakeSysStdin())
    ...
    a2a_mod._cmd_start(_Args())
    cmd = captured["cmd"]
    assert "--token" not in cmd, f"token must not be an argument: {cmd}"
    stdin = captured["stdin"]
    assert stdin.data == b"motdepasse-agent-1\njeton-secret-123\n"


def test_a2a_start_stale_pid_file_restarts(cli_env, monkeypatch):
    """m11: a live PID whose bridge does not answer must NOT report
    'already running' — the bridge is restarted (a zombie or a foreign
    process holding the PID file would otherwise fake a running
    bridge)."""
    import synapse.cli.a2a as a2a_mod
    ...
    monkeypatch.setattr(a2a_mod, "socket_responds", lambda *a, **k: True)
    monkeypatch.setattr(a2a_mod, "resolve_config", lambda *a, **k: None)
    monkeypatch.setattr(a2a_mod, "read_pid_file", lambda *a, **k: {"pid": 99999})
    monkeypatch.setattr(a2a_mod, "_alive", lambda pid: True)
    monkeypatch.setattr(a2a_mod, "_a2a_responding", lambda *a, **k: False)
    monkeypatch.setattr(a2a_mod, "wait_ready", lambda *a, **k: True)
    monkeypatch.setattr(a2a_mod, "getpass_get", lambda prompt: "motdepasse-agent-1")
    monkeypatch.setattr(a2a_mod.sys, "stdin", _FakeStdin())
    ...
    a2a_mod._cmd_start(_Args())
    assert started["spawned"], "a stale PID file must restart the bridge"

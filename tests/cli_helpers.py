"""Shared helpers for the CLI tests (not collected by pytest).

The unified CLI is tested in subprocesses: disposable configuration resolved
via ``$Synapse_CONFIG`` (SPEC_CLI §2 search order), execution of
``synapse <args...>`` with controlled stdin/env.

Port isolation: ``SYNAPSE_WEB_PORT`` / ``SYNAPSE_A2A_PORT`` are set to
random free ports — production (or any other session) can listen on the
default ports (8080/8090) without failing the tests (SPEC_PRODUCTION §10.5).
"""

from __future__ import annotations

import json
import os
import signal
import socket
import subprocess
import sys
import tempfile
import time

from synapse.config import Config

CLI = [sys.executable, "-m", "synapse.cli"]


def _free_port() -> int:
    """Random free TCP port on 127.0.0.1 (reduced race window)."""
    with socket.socket(socket.AF_INET, socket.SOCK_STREAM) as sock:
        sock.bind(("127.0.0.1", 0))
        return sock.getsockname()[1]


def cli_env_data(tmp_path):
    """(config, config_file, env): isolated JSON config + environment."""
    conf = {
        "storage_dir": str(tmp_path / "data"),
        "socket_path": str(tmp_path / "run" / "synapse.sock"),
        "log_dir": str(tmp_path / "logs"),
        "backup_dir": str(tmp_path / "backups"),
    }
    config = Config.from_dict(conf)
    config_file = tmp_path / "config.json"
    config_file.write_text(json.dumps(conf))
    env = dict(os.environ)
    env["Synapse_CONFIG"] = str(config_file)
    # The tests simulate a development environment (no systemd supervision):
    # the real units of the production machine must not switch `update apply`
    # over to systemctl.
    env["SYNAPSE_NO_SYSTEMD"] = "1"
    # Parent watch (auditor F1): a daemon started by a test exits when
    # its pytest worker dies — a killed worker never orphans a daemon.
    env["SYNAPSE_WATCH_PARENT"] = "1"
    # Isolated HTTP ports: never collide with production.
    env["SYNAPSE_WEB_PORT"] = str(_free_port())
    env["SYNAPSE_A2A_PORT"] = str(_free_port())
    # Parallel workers (pytest -n 3) compete for CPU/RAM: the CLI's default
    # 15s start timeout is too tight under load. Give test-started daemons
    # headroom (production default is untouched).
    env["SYNAPSE_START_TIMEOUT"] = "60"
    # Same for the client socket read timeout (10s default): a request can
    # exceed it on a loaded machine — the bridge would surface it as an
    # internal error.
    env["SYNAPSE_SOCKET_TIMEOUT"] = "60"
    return config, str(config_file), env


def run_cli(env, *args, stdin: str = "", timeout: int = 60):
    """Runs ``synapse <args...>`` in a subprocess; returns the
    ``CompletedProcess`` (stdout/stderr captured)."""
    return subprocess.run(
        CLI + [*args], input=stdin.encode(), capture_output=True,
        env=env, timeout=timeout,
    )


def _daemon_config(path: str) -> str | None:
    """The ``--config`` value of a ``_daemon`` command line, or None."""
    parts = path.split("\x00")
    if "_daemon" not in parts or "--config" not in parts:
        return None
    return parts[parts.index("--config") + 1]


def daemon_pids_for(config_path: str) -> list[int]:
    """PIDs of the ``_daemon`` processes serving the given config."""
    config_path = os.path.abspath(config_path)
    found = []
    for entry in os.listdir("/proc"):
        if not entry.isdigit():
            continue
        try:
            with open(f"/proc/{entry}/cmdline", "rb") as fh:
                raw = fh.read().decode("utf-8", errors="replace")
        except OSError:
            continue  # process exited between listdir and read
        if _daemon_config(raw) == config_path:
            found.append(int(entry))
    return found


def sweep_orphan_test_daemons() -> int:
    """SIGTERMs leftover ``_daemon`` processes whose config lives under
    the system temp dir (test configs only — production configs live in
    ``~/.local/share/synapse`` and are never touched). Returns the number
    of processes terminated. Orphans are left by pytest workers killed
    before the parent-watch existed (auditor F1)."""
    tmp = os.path.realpath(tempfile.gettempdir())
    killed = 0
    for entry in os.listdir("/proc"):
        if not entry.isdigit() or entry == str(os.getpid()):
            continue
        try:
            with open(f"/proc/{entry}/cmdline", "rb") as fh:
                raw = fh.read().decode("utf-8", errors="replace")
            cfg = _daemon_config(raw)
        except OSError:
            continue
        if cfg and os.path.realpath(cfg).startswith(tmp):
            pid = int(entry)
            try:
                os.kill(pid, signal.SIGTERM)
                killed += 1
            except ProcessLookupError:
                pass
    if killed:
        # Give the daemons a grace period to stop cleanly (PID file
        # removal), then force-kill what remains.
        time.sleep(1.0)
        for entry in os.listdir("/proc"):
            if not entry.isdigit():
                continue
            try:
                with open(f"/proc/{entry}/cmdline", "rb") as fh:
                    raw = fh.read().decode("utf-8", errors="replace")
                cfg = _daemon_config(raw)
            except OSError:
                continue
            if cfg and os.path.realpath(cfg).startswith(tmp):
                try:
                    os.kill(int(entry), signal.SIGKILL)
                except ProcessLookupError:
                    pass
    return killed

...
def test_constraint_cross_org_task_delegation_bypass(fx):
    """Cross-org task creation/transfer/delegation must respect the
    external-communication policies, exactly like messaging. A closed
    org must not be able to push a task or a delegation to a foreign
    agent (permissions audit F1 HIGH)."""
    create_organization(fx.config, ORG2_NAME, ORG2_PASSWORD, ORG2_PASSWORD)
    fx.client.create_agent("dave", "motdepasse-dave-1", "Agent dave", ORG2_NAME, ORG2_PASSWORD)
    # both orgs are closed by default → POLICY_DENIED for cross-org task
    with pytest.raises(ApiClientError) as exc:
        fx.client.create_task(
            "Process secret data", "dave", ALICE, ALICE_PASSWORD,
        )
    assert exc.value.code == "POLICY_DENIED"
    ...
    task = fx.client.create_task(
        "Legit cross-org task", "dave", ALICE, ALICE_PASSWORD,
    )
    assert task["assignee_username"] == "dave"
    # re-open only one side → refused again (both policies must allow)
    fx.client.set_organization_policy(False, True, ORG2_NAME, ORG2_PASSWORD)
    with pytest.raises(ApiClientError) as exc:
        fx.client.create_task(
            "Blocked again", "dave", ALICE, ALICE_PASSWORD,
        )
    assert exc.value.code == "POLICY_DENIED"
    # delegation to a foreign agent also gated by the policy
    fx.client.set_organization_policy(True, True, ORG2_NAME, ORG2_PASSWORD)
    task_id = task["task_id"]
    # close outgoing on the delegator's side → delegation blocked
    fx.client.set_organization_policy(True, False, ORG_NAME, ORG_PASSWORD)
    with pytest.raises(ApiClientError) as exc:
        fx.client.create_delegation(
            task_id, "dave",
            "2099-12-31T23:59:59.999Z", ALICE, ALICE_PASSWORD,
        )
    assert exc.value.code == "POLICY_DENIED"

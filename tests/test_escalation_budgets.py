...
def test_budget_partial_update_preserves_other_limit(fx):
    """Setting only one budget must not silently erase the other one:
    the ON CONFLICT update keeps the existing value when the new one is
    null (the CLI sends None for the flag that was not provided)."""
    fx.client.set_agent_budget(BOB, ORG_NAME, ORG_PASSWORD, max_active_tasks=1)
    # only the message limit is provided now — the task limit must survive
    fx.client.set_agent_budget(BOB, ORG_NAME, ORG_PASSWORD, max_messages_per_hour=5)
    fx.client.create_task("A", BOB, ALICE, ALICE_PASSWORD)
    with pytest.raises(ApiClientError) as exc:
        fx.client.create_task("B", BOB, ALICE, ALICE_PASSWORD)
    assert exc.value.code == QUOTA_EXCEEDED  # max_active_tasks=1 still applies
    # the message limit is also still applied
    for i in range(5):
        fx.send(BOB, BOB_PASSWORD, ALICE, f"m{i}", f"cmid-part-{i}")
    with pytest.raises(ApiClientError) as exc:
        fx.send(BOB, BOB_PASSWORD, ALICE, "m5", "cmid-part-5")
    assert exc.value.code == QUOTA_EXCEEDED


def test_budget_zero_rejected(fx):
    """Spec F9: budgets are integers >= 1; 0 is refused (INVALID_ARGUMENT)
    — the CLI help must not claim that 0 removes the limits."""
    with pytest.raises(ApiClientError) as exc:
        fx.client.set_agent_budget(BOB, ORG_NAME, ORG_PASSWORD, max_active_tasks=0)
    assert exc.value.code == "INVALID_ARGUMENT"


def test_escalation_respects_target_budget(fx):
    """F9: automatic escalation must respect the target's active-task
    budget — an escalator at max_active_tasks must not receive more."""
    fx.client.set_agent_budget(ALICE, ORG_NAME, ORG_PASSWORD, max_active_tasks=1)
    # alice already has one active task (the escalated one must not arrive)
    fx.client.create_task("Occupe alice", ALICE, BOB, BOB_PASSWORD)
    fx.client.set_escalation_policy(True, 1, 1, ALICE, ORG_NAME, ORG_PASSWORD)
    late = fx.client.create_task("En retard budget", BOB, ALICE, ALICE_PASSWORD,
                                 due_at="2020-01-01T00:00:00.000Z")
    fx.client.create_task("Trigger", BOB, ALICE, ALICE_PASSWORD)
    # the late task stays with bob: alice's budget is reached
    got = fx.client.get_task(late["task_id"], ALICE, ALICE_PASSWORD)
    assert got["assignee_username"] == BOB
    assert fx.client.get_events(ALICE, ALICE_PASSWORD, types=["task.escalated"])["events"] == []

...
def test_update_check_no_channel_is_unknown_not_up_to_date(cli_env):
    """M11: without a remote channel, up_to_date must be None (unknown),
    never a fake True — 'nothing to compare' is not 'up to date'."""
    _, _, env = cli_env
    env2 = dict(env)
    env2.pop("SYNAPSE_UPDATE_URL", None)
    proc = run_cli(env2, "update", "check", "--json")
    assert proc.returncode == 0, proc.stderr.decode()
    data = json.loads(proc.stdout.decode())["data"]
...
def test_update_check_payload_without_version_is_unknown(cli_env, tmp_path):
    """M11: a channel payload without a version field must not produce
    the contradictory {remote: <payload>, up_to_date: false} — the state
    is unknown (None)."""
    _, _, env = cli_env
    latest = tmp_path / "latest-no-version.json"
    latest.write_text(json.dumps({"other": "field"}))
    env2 = dict(env)
    env2["SYNAPSE_UPDATE_URL"] = latest.as_uri()
    proc = run_cli(env2, "update", "check", "--json")
    assert proc.returncode == 0, proc.stderr.decode()
...
# ---------------------------------------------------------------------------
# Bare ``synapse update`` (check + apply in one step — new in 3.1.6)
# ---------------------------------------------------------------------------


def test_update_simple_already_up_to_date_no_action(cli_env, tmp_path):
    """Bare update: same versions → 'already up to date', exit 0,
    NO action (no backup, no command, no restart)."""
    _, config_file, env = cli_env
    _bootstrap(env)
    try:
        marker = tmp_path / "upgraded.marker"
        ...
        from importlib.metadata import version as _pkg_version
        latest = tmp_path / "latest.json"
        latest.write_text(json.dumps({"version": _pkg_version("synapse-messenger")}))
        env2 = dict(env)
        env2["SYNAPSE_UPDATE_URL"] = latest.as_uri()
        env2["SYNAPSE_UPDATE_COMMAND"] = f"sh {script}"

        proc = run_cli(env2, "update")
        assert proc.returncode == 0, proc.stderr.decode()
        out = proc.stdout.decode()
        assert "Already up to date" in out
        # NO action: no marker, no backup, server still running
...
def test_update_simple_available_confirmed_applies(cli_env, tmp_path):
    """Bare update: newer remote version + confirmation → the full apply
    path runs (backup → stop → command → restart) and the new version is
    confirmed."""
    _, config_file, env = cli_env
    _bootstrap(env)
    try:
        ...
        proc = run_cli(env2, "update", "--yes", stdin="y\n")
        assert proc.returncode == 0, proc.stderr.decode()
        out = proc.stdout.decode()
        assert "Update available" in out
        assert "Update applied." in out
        assert "Now installed:" in out
        # the command ran (marker written) and the backup was created
        assert marker.read_text() == "3.1.6-simule"
        backups = list((tmp_path / "backups").glob("*.synbk"))
        assert len(backups) == 1
...
def test_update_simple_declined_does_nothing(cli_env, tmp_path):
    """Bare update: newer version but the operator declines → canceled,
    no action."""
    _, config_file, env = cli_env
    _bootstrap(env)
    try:
        ...
        proc = run_cli(env2, "update", stdin="n\n")
        assert proc.returncode == 0, proc.stderr.decode()
        out = proc.stdout.decode()
        assert "Update available" in out
        assert "Update canceled." in out
        assert not marker.exists()
        assert not list((tmp_path / "backups").glob("*.synbk"))
    finally:
        run_cli(env, "server", "stop")
...
def test_update_simple_check_only_equals_check(cli_env, tmp_path):
    """--check-only: equivalent to 'update check' (JSON payload)."""
    _, config_file, env = cli_env
    _bootstrap(env)
    try:
        latest = tmp_path / "latest.json"
        ...
        proc = run_cli(env2, "update", "--check-only", "--json")
        assert proc.returncode == 0, proc.stderr.decode()
        data = json.loads(proc.stdout.decode())["data"]
        assert data["remote"] == "3.2.0"
        assert data["up_to_date"] is False

...
def test_create_org_hashes_password_outside_transaction(fx, monkeypatch):
    """m3: the expensive Argon2id hash must run BEFORE the write
    transaction (the global write lock must not be held during the
    computation) — same pattern as create_agent."""
    import synapse.service as service_mod
    import synapse.db as db_mod
    from synapse.db import connect

    calls = []
    real_begin = db_mod.begin_immediate

    def _recording_begin(conn):
        calls.append("lock")
        return real_begin(conn)

    monkeypatch.setattr(db_mod, "begin_immediate", _recording_begin)

    def _counting_hash(_pwd):
        calls.append("hash")
        return "x" * 97  # Argon2id-style placeholder

    monkeypatch.setattr(service_mod, "hash_password", _counting_hash)
    monkeypatch.setattr(service_mod, "human_password_sentinel", lambda: "s" * 97)

    with connect(fx.config) as conn:
        service_mod._human_create_org(
            fx.server.service, conn,
            {"organization_name": "org_hash_ordre", "organization_password": "mdp"},
            HUMAN,
        )
    assert calls == ["hash", "lock"], f"hash must precede the lock: {calls}"

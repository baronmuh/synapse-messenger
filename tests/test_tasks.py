...
def test_task_assignable_cross_organization(fx):
    """Documented F5 deviation (SPEC.txt §V.7 bis): a task can be assigned to
    any active account, including one from another organization — communication
    policies apply only to messages, not to coordination objects."""
    ...
    # alice (root_org, closed by default) assigns a task to dave (second_org)
